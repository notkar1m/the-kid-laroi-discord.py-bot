...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop, download=False):
        ...
        embed = discord.Embed(title=f"{data['title']}",
            description=f"Was added  to the Queue.",
            url=f'{data["webpage_url"]}')
...
class Music(commands.Cog):
	...
	async def play_(self, ctx, *, search: str):
		...
		if search[0:3] == "http":
			source = await YTDLSource.create_source(ctx, loop=self.bot.loop, download=False)
		else:
	...
	#     vc.stop()
	#     await ctx.send(f'**`{ctx.author}`**: Skipped All Songs!')
